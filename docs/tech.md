...
- **Scheduler (внутри бота)**: `python-telegram-bot` JobQueue (использует APScheduler, если установлен)
- **Конфиг**: `.env` (опционально через `python-dotenv`) + `os.getenv` (без pydantic)
...
- **Fixes (XLSX)**: “пакеты правок” (preview/apply) в копию файла
- **Issue History**: история проблем для “не исправлено N дней”
...
### Local XLSX (текущий MVP)
- чтение локальной выгрузки через `openpyxl` (`src/local/xlsx_reader.py`)
- команды бота запускают `run_xlsx_audit`
- “apply” правок делается **только в копию XLSX**, оригинал не трогаем

### Google Sheets (этап 3)
- чтение по service account
- (позже) запись правок после подтверждения пользователя
...
## Периодические отчёты и “не исправлено N дней”

- еженедельный отчёт на 3‑й рабочий день недели (обычно среда) планируется внутри бота JobQueue
- история проблем хранится в JSON (`out/issue_history.json` или `ISSUE_HISTORY_PATH`)
- в отчёте показываем критичные (red) проблемы, которые держатся ≥ `PERSISTENT_RED_DAYS`
- дополнительно хранится снапшот прошлого отчёта (`out/issue_snapshot.json`) и показывается дельта:
